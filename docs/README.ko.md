...
생성은 명령이 됐지만, **만들어진 Context를 Worker에게 전달하는 일은 여전히 사람이 한다.**
나머지 네 전이도 아직 명령이 없다.
...
**`task context`가 Context Package를 만들지만 전달하지는 않는다.** 사람이 결과를
복사해 Worker에게 붙여넣는 단계가 남아 있고, 그것을 없애는 것이 다음 목표다.

Task를 만들거나 목록을 보는 명령, 새 프로젝트에 `.bcos/` 구조를 만드는 `init`,
상태를 조회하는 `status`와 인덱스를 다시 만드는 `reindex`도 아직 없다.
Codex나 Claude를 자동으로 실행하는 연결도 없다.

구현 시점은 약속하지 않는다. 필요가 확인된 순서대로 만든다.

## T-001부터 T-007까지

지금까지 일곱 개의 작업이 프로토콜 전 과정을 통과했다.
...
일곱 작업 모두 재작업 없이 한 번에 승인됐고, 범위 이탈과 과설계 위반은 0건이었다.

다만 이 수치들을 생산성 향상률로 환산하지는 않는다. 일곱 작업은 성격이 전부 다르고
...
핵심 세 단계와 Context 생성이 끝났다. 그래서 우선순위를 바꿨다.

남은 전이 네 개를 마저 채우는 것은 이미 검증된 작업의 반복이다. 그보다 **모델을 바꿀 때
사람이 프로젝트를 처음부터 다시 설명하는 문제**가 더 크다. 다음은 Worker 실행,
그다음이 프롬프트 조립과 실행기 교체다.
